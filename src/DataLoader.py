...
letters = '\ !%"#&\'()*+,-./0123456789:;?AÁẢÀÃẠÂẤẨẦẪẬĂẮẲẰẴẶBCDĐEÉẺÈẼẸÊẾỂỀỄỆFGHIÍỈÌĨỊJKLMNOÓỎÒÕỌÔỐỔỒỖỘƠỚỞỜỠỢPQRSTUÚỦÙŨỤƯỨỬỪỮỰVWXYÝỶỲỸỴZaáảàãạâấẩầẫậăắẳằẵặbcdđeéẻèẽẹêếểềễệfghiíỉìĩịjklmnoóỏòõọôốổồỗộơớởờỡợpqrstuúủùũụưứửừữựvwxyýỷỳỹỵz'
MAX_LEN = 100
SIZE = 800, 64
...
class VizCallback(keras.callbacks.Callback):
    def __init__(self, sess, y_func, text_img_gen, text_size, num_display_words=1):
        self.y_func = y_func
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        for i in range(min(self.num_display_words, len(inputs))):
            ...
            print("lable shape: {}".format(batch['the_labels'][:self.num_display_words].shape))
            print("input_length: {}".format(batch['input_length'][:self.num_display_words]))
            print("lable length: {}".format(batch['label_length'][:self.num_display_words]))

            print("output shape: {}".format(self.y_func([inputs])[0].shape))
            print("text_size: {}".format(self.text_size))
...
class TextImageGenerator:
    def __init__(self, img_dirpath, labels_path, img_w, img_h,
        ...
        self.labels = json.load(
            open(labels_path)) if labels_path != None else None
        self.img_dir =[fn for fn in os.listdir(self.img_dirpath) if fn.endswith('png')]     # images list
        ...
        self.cur_index = 0
        self.imgs = np.zeros(
            (self.n, self.img_w, self.img_h, 1), dtype=np.float16)
    ...
    def build_data(self):
        ...
        for i, img_file in enumerate(self.img_dir):
            #img = image.load_img(self.img_dirpath +
            #                     img_file, target_size=SIZE[::-1])
            #img = image.img_to_array(img)
            #img = preprocess_input(img).astype(np.float16)
            img = cv2.imread(self.img_dirpath + img_file, cv2.IMREAD_GRAYSCALE).reshape(self.img_w, self.img_h, 1).astype(float)
            self.imgs[i] = img
            if self.labels != None:
